...
public class SurfaceTextureHelper {
    ...
    private FrameSendEvents mFrameSendEvents;

    private volatile boolean isSendFirstFrame = false;


    public void isAutoRefresh(boolean isAutoRefresh) {
    ...
    public static SurfaceTextureHelper create(final String threadName,
        ...
        final HandlerThread thread = new HandlerThread(threadName, Process.THREAD_PRIORITY_VIDEO);
        thread.start();
    ...
    public void setFrameSendListener(FrameSendEvents frameSendEvents) {
        this.mFrameSendEvents = frameSendEvents;
    }
...
//        Log.w(TAG, "tryDeliverTextureFrame here");
        isTextureInUse = true;
        hasPendingTexture = false;
        updateTexImage();
        ...
        if (!isSendFirstFrame && mFrameSendEvents != null) {
            mFrameSendEvents.onFirstFrameSend();
